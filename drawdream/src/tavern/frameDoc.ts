/**
 * srcdoc 组装与程序卡识别（参考梨园 Liyuan frameDoc.ts，clean-room 实现）。
 *
 * 关键点：
 * - 卡脚本内 `</script>` 截断修复：状态机跳过 JS 字符串/模板/注释里的字面 `</script`
 * - 程序卡检测：大体积脚本 或 position:fixed + 100vh → iframe 锁视口高度
 * - 高度上报：量「内容盒子」而非 documentElement.scrollHeight，避免 100vh 反馈环
 */
...
/**
 * 是否「全屏程序卡」（position:fixed 铺满的卡内 UI）。
 * 判定（满足其一）：
 * - 体量很大（≥25KB 脚本界面）
...
// 与 DrawDream 主界面一致的字体栈（iframe 默认字体不美观，需显式指定）
const SEAMLESS_BODY_FONT =
	`font-family:'AaGuDianKeBenSongYouMoBan','AaGuDianKeBenSong','Songti SC','Noto Serif SC','STSong',serif;` +
	`font-size:14px;line-height:1.6;color:#3f3f3f;`;
...
/** 无痕·整页文档：禁止 html/body 吃满 100vh（避免白底无限向下扩的反馈环） */
...
/** 非 seamless（静态/调试帧）基础样式：至少保证字体与排版不塌 */
...
/**
 * 高度上报：量 body 直接子元素的底部，忽略 script/style；
 * ResizeObserver + 前几秒轮询，稳定后停（避免 100vh 蠕变反馈环）。
 */
export const HEIGHT_REPORTER_SNIPPET =
	...
	`last=h;stable=0;` +
	`parent.postMessage({drawdreamFrameHeight:h,frameId:window.name},"*");` +
	`}` +
 ...
 * 跳过 JS 字符串/模板/注释里的字面量 `</script`（程序卡把整页 HTML 塞进模板字符串时，
 * HTML 解析器会在此截断 → 主脚本只剩部分 → 按钮无监听）。
 */
...
/**
 * 组装 srcdoc。桥 snippet 必须先于卡脚本注入（保证 TavernHelper/eventOn 在初始化时已存在）。
 * heightReporter 插在最后一个 </body> 前。
 */
export function buildSrcDoc(
	html: string,
	scripts: boolean,
	seamless: boolean,
	bridgeSnippet: string,
	extraCode = "",
): string {
	...
	const bridge = scripts ? bridgeSnippet : "";
	// 卡扩展脚本代码：包裹 try/catch 防单条错误拖垮整体
	const extTag = scripts && extraCode ? `<script>try{${extraCode}}catch(e){console.error('[CardScript]',e)}</script>` : "";
	const head =
		`<meta charset="utf-8">` +
		`<meta http-equiv="Content-Security-Policy" content="${csp}">` +
		`<style>${seamless ? seamlessCss : BASE_FRAME_CSS}</style>` +
		bridge +
		extTag;
