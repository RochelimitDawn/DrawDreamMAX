...
import type {
	...
	ThinkingLevel,
} from "@drawdream/agent-core";
import type { AssistantMessage, ImageContent, Message, Model, TextContent } from "@drawdream/ai/compat";
import {
	clampThinkingLevel,
...
export class AgentSession {
	...
	private _isRetryableError(message: AssistantMessage): boolean {
		...
		// 已输出可见正文时重试会让用户看到完整内容后再次生成。
		// 保留当前部分结果，把重试限定在首字前失败。
		if (
			message.content.some(
				(content) => content.type === "text" && (content as TextContent).text.trim().length > 0,
			)
		) {
			return false;
		}
