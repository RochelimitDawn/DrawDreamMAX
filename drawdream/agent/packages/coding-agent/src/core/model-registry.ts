...
} from "@drawdream/ai/compat";
import { registerOAuthProvider, resetOAuthProviders } from "@drawdream/ai/oauth";
...
export type ModelRegistryOptions = {
	/**
	 * 跳过内置全量 provider 目录（openrouter/bedrock 等）。
	 * DrawDream Web 产品仅用 models.json 自定义渠道时开启，显著降低冷启动。
	 */
	skipBuiltInModels?: boolean;
};

/** CLI 全量入口绑定；Web 入口不绑定 → 永不加载 models.generated */
type BuiltinCatalog = {
	getProviders: () => string[];
	getModels: (provider: KnownProvider) => Model<Api>[];
};

let builtinCatalog: BuiltinCatalog | null = null;

/** 由完整 barrel（index.ts）在加载时调用；web.ts 不调用 */
export function bindBuiltinModelCatalog(catalog: BuiltinCatalog): void {
	builtinCatalog = catalog;
}

function resolveBuiltinCatalog(): BuiltinCatalog {
	if (builtinCatalog) return builtinCatalog;
	throw new Error(
		"内置模型目录未绑定。CLI 请从 @drawdream/agent-runtime 导入；Web 请 skipBuiltInModels 或 DRAWDREAM_SKIP_BUILTIN_MODELS=1",
	);
}
...
export class ModelRegistry {
	...
	private skipBuiltInModels: boolean;
	...
	private constructor(
		...
		this.skipBuiltInModels =
			options?.skipBuiltInModels === true ||
			process.env.DRAWDREAM_SKIP_BUILTIN_MODELS === "1" ||
			process.env.DRAWDREAM_SKIP_BUILTIN_MODELS === "true";
		this.loadModels();
	}
	...
	private loadBuiltInModels(
		...
		// DrawDream Web：仅 models.json 自定义渠道，跳过 openrouter/bedrock 等全量目录物化
		if (this.skipBuiltInModels) return [];
		const { getProviders, getModels } = resolveBuiltinCatalog();
	...
	private validateConfig(config: ModelsConfig): void {
		const builtInProviders = new Set<string>(
			this.skipBuiltInModels ? [] : resolveBuiltinCatalog().getProviders(),
		);
	...
	private parseModels(config: ModelsConfig): Model<Api>[] {
		...
		const catalog = this.skipBuiltInModels ? null : resolveBuiltinCatalog();
		const builtInProviders = new Set<string>(catalog ? catalog.getProviders() : []);
		...
		const getBuiltInDefaults = (providerName: string): { api: string; baseUrl: string } | undefined => {
			if (!catalog || !builtInProviders.has(providerName)) return undefined;
			if (builtInDefaultsCache.has(providerName)) return builtInDefaultsCache.get(providerName);
			const builtIn = catalog.getModels(providerName as KnownProvider) as Model<Api>[];
