import type { AgentTool } from "@drawdream/agent-core";
import { fauxAssistantMessage, fauxThinking, fauxToolCall } from "@drawdream/ai";
...
describe("AgentSession retry and event characterization", () => {
	...
	it("does not retry a transient error after visible text was produced", async () => {
		const harness = await createHarness({ settings: { retry: { enabled: true, maxRetries: 3, baseDelayMs: 1 } } });
		harnesses.push(harness);
		harness.setResponses([
			fauxAssistantMessage("已经生成的正文", { stopReason: "error", errorMessage: "stream ended before message_stop" }),
			fauxAssistantMessage("不应出现的重试正文"),
		]);
