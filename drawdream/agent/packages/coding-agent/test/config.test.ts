...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@drawdream/agent-runtime")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @drawdream/agent-runtime",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@drawdream/agent-runtime")).toBeUndefined();
		expect(getUpdateInstruction("@drawdream/agent-runtime")).toBe(
			"Update @drawdream/agent-runtime using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@drawdream/agent-runtime");
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@drawdream/agent-runtime",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @drawdream/agent-runtime`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@drawdream/agent-runtime", undefined, {
			packageName: "@drawdream/agent-runtime",
			installSpec: "@drawdream/agent-runtime@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@drawdream/agent-runtime@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @drawdream/agent-runtime@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@drawdream/agent-runtime", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @drawdream/agent-runtime`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@drawdream/agent-runtime")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @drawdream/agent-runtime",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@drawdream/agent-runtime"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @drawdream/agent-runtime",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@drawdream/agent-runtime";
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@drawdream/agent-runtime")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@drawdream/agent-runtime")).toContain(
