# @drawdream/agent-core

Stateful agent with tool execution and event streaming. Built on `@drawdream/ai`.

## Installation

```bash
npm install @drawdream/agent-core
```

## Quick Start

```typescript
import { Agent } from "@drawdream/agent-core";
import { getModel } from "@drawdream/ai";
...
```typescript
declare module "@drawdream/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@drawdream/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@drawdream/agent-core";
