...
```bash
npm install @drawdream/ai
```

TypeBox exports are re-exported from `@drawdream/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@drawdream/ai';
import { builtinModels } from '@drawdream/ai/providers/all';
...
```typescript
import { anthropicProvider } from '@drawdream/ai/providers/anthropic';
import { openaiProvider } from '@drawdream/ai/providers/openai';
import { openrouterProvider } from '@drawdream/ai/providers/openrouter';
import { amazonBedrockProvider } from '@drawdream/ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@drawdream/ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@drawdream/ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@drawdream/ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@drawdream/ai';
...
```typescript
import { validateToolCall, type Tool } from '@drawdream/ai';
...
```typescript
import { builtinImagesModels } from '@drawdream/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@drawdream/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@drawdream/ai';
import { openAICompletionsApi } from '@drawdream/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@drawdream/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@drawdream/ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@drawdream/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@drawdream/ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@drawdream/ai';
import { anthropicProvider } from '@drawdream/ai/providers/anthropic';
import { openaiProvider } from '@drawdream/ai/providers/openai';
import { googleProvider } from '@drawdream/ai/providers/google';
...
```typescript
import { createModels } from '@drawdream/ai';
import { anthropicProvider } from '@drawdream/ai/providers/anthropic';
...
- `@drawdream/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@drawdream/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@drawdream/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@drawdream/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@drawdream/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@drawdream/ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@drawdream/ai/bedrock-provider';
...
```bash
npx @drawdream/ai login              # interactive provider selection
npx @drawdream/ai login anthropic    # login to specific provider
npx @drawdream/ai list               # list available providers
```
...
```typescript
// Before
import { getModel, complete } from '@drawdream/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@drawdream/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@drawdream/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@drawdream/ai`.
