 ...
 *
 * Existing apps switch imports from "@drawdream/ai" to
 * "@drawdream/ai/compat" unchanged; new code uses `createModels()`
...
/** @deprecated Static catalog read. Use `getBuiltinModel` from "@drawdream/ai/providers/all" or `Models.getModel()`. */
export const getModel = getBuiltinModel;

/** @deprecated Static catalog read. Use `getBuiltinModels` from "@drawdream/ai/providers/all" or `Models.getModels()`. */
export const getModels = getBuiltinModels;

/** @deprecated Static catalog read. Use `getBuiltinProviders` from "@drawdream/ai/providers/all" or `Models.getProviders()`. */
...
function shouldUseBuiltinModels(model: Model<Api>): boolean {
	const builtin = getCompatModels().getModel(model.provider, model.id);
...
export function stream<TApi extends Api>(
	...
	if (shouldUseBuiltinModels(model)) {
		return getCompatModels().stream(model, context, options as ApiStreamOptions<TApi> | undefined);
	}
