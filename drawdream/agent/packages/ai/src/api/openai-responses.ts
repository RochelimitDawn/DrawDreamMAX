...
function createClient(
	...
	return new OpenAI({
		apiKey,
		// SDK 会在 baseURL 后追加 /responses；用户若粘贴 …/v1/responses 需剥掉后缀
		baseURL: normalizeOpenAiResponsesBaseUrl(model.baseUrl),
		dangerouslyAllowBrowser: true,
		defaultHeaders: headers,
	});
}

/** 归一 OpenAI Responses 的 baseURL：去掉尾斜杠与误带的 /responses */
export function normalizeOpenAiResponsesBaseUrl(baseUrl: string): string {
