...
import { emptyBranchStateIndex, recordBranchState, restoreBranchState } from "../src/branch-state.ts";
...
test("branch state restores the latest snapshot on the selected branch", () => {
	const base = { time: "", location: "", chapter: "", characters: {}, inventory: [], flags: {}, plot_threads: [] };
	const index = recordBranchState(
		recordBranchState(emptyBranchStateIndex(), "a1", { ...base, location: "旧路" }, 1),
		"a2",
		{ ...base, location: "新路" },
		2,
	);
	assert.equal(restoreBranchState(index, ["root", "a1"], base).location, "旧路");
	assert.equal(restoreBranchState(index, ["root", "a2"], base).location, "新路");
	assert.equal(restoreBranchState(index, ["root", "other"], base).location, "");
});
