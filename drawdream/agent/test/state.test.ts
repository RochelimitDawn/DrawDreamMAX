...
test("注入格式化", () => {
	...
	assert.ok(text.includes("第一章 · 林间"));
	assert.ok(text.includes("Alice"));
	assert.ok(text.includes("药茶"));
	assert.ok(formatState(defaultState()).includes("尚无记录"));
});

test("补丁：chapter 字符串替换", () => {
	let s = defaultState();
	assert.equal(s.chapter, "");
	s = applyPatch(s, { chapter: "第二幕 · 听雨" }).state;
	assert.equal(s.chapter, "第二幕 · 听雨");
	s = applyPatch(s, { chapter: "第三幕" }).state;
	assert.equal(s.chapter, "第三幕");
});
