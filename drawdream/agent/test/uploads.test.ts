...
test("saveUpload writes under .drawdream-uploads and never overwrites", () => {
	const dir = mkdtempSync(join(tmpdir(), "dd-up-"));
	try {
		const a = saveUpload(dir, "map.png", Buffer.from("aaa"));
		assert.match(a.file, /^\.drawdream-uploads\/\d{8}-\d{6}-map\.png$/);
		assert.equal(a.bytes, 3);
		const b = saveUpload(dir, "map.png", Buffer.from("bbbb"));
		assert.notEqual(a.file, b.file);
		const list = listUploads(dir);
		assert.ok(list.every((u) => u.file.startsWith(".drawdream-uploads/")));
...
test("formatUploadIndex empty returns null; truncates with note", () => {
	...
	const u = (name: string) => ({ name, file: `.drawdream-uploads/${name}`, bytes: 1024, mtimeMs: 0 });
