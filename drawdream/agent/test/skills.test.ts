...
test("saveSkill and listSkills roundtrip", () => {
	const dir = mkdtempSync(join(tmpdir(), "dd-sk-"));
	try {
		...
		assert.equal(first.file, ".drawdream-skills/draw.md");
...
test("formatSkillIndex", () => {
	const text = formatSkillIndex([{ name: "draw", description: "comfy", file: ".drawdream-skills/draw.md" }]);
	assert.match(text, /draw/);
	assert.match(text, /comfy/);
	const hidden = formatSkillIndex([
		{ name: "secret", description: "d", file: ".drawdream-skills/secret.md", disableModelInvocation: true },
	]);
