...
test("isStPresetJson / summarizeConvertReport", () => {
	assert.equal(isStPresetJson({ prompts: [] }), true);
	assert.equal(isStPresetJson({ prompt_order: [] }), true);
	assert.equal(isStPresetJson({ name: "x", blocks: [] }), false);
	assert.equal(isStPresetJson(null), false);

	const { preset, report } = convertStPreset(stPreset as Record<string, unknown>, "测");
	const s = summarizeConvertReport(report, preset);
	assert.equal(s.blockCount, preset.blocks.length);
	assert.ok(s.system >= 1);
	assert.ok(s.postHistory >= 1);
	assert.ok(s.marker >= 1);
	assert.ok(s.disabled >= 1);
	assert.ok(s.missing >= 1);
	assert.ok(s.samplerKeys.includes("temperature"));
});
