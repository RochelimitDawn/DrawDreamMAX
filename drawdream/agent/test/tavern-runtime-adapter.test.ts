...
test('external module policy requires HTTPS, declaration and explicit grant', async () => {
  ...
  const records = recordModuleCache([], { url: 'https://example.com/x.js', fingerprint: 'sha256:a', cachedAt: 1, permission: 'allow' })
  assert.equal(findModuleCache(records, 'https://example.com/x.js', 'sha256:a')?.permission, 'allow')
  assert.equal(findModuleCache(records, 'https://example.com/x.js', 'sha256:b'), null)
})
