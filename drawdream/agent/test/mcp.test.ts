...
test("mcp config roundtrip + allocate id", () => {
	const dir = mkdtempSync(join(tmpdir(), "drawdream-mcp-"));
	try {
		resetMcpHubForTests();
		assert.deepEqual(loadMcpConfig(dir), emptyMcpConfig());
		// 用不太可能与本机 Claude 台账撞名的 id
		const id = allocateServerId(dir, "drawdream-unique-mcp-zzz");
		assert.equal(id, "drawdream_unique_mcp_zzz");
		saveMcpConfig(dir, {
		...
		assert.equal(loaded.servers[0].id, "drawdream_unique_mcp_zzz");
		assert.equal(allocateServerId(dir, "drawdream-unique-mcp-zzz"), "drawdream_unique_mcp_zzz_2");
...
test("discoverMcpCatalog merges project .mcp.json and drawdream-mcp; defaults off", () => {
	const dir = mkdtempSync(join(tmpdir(), "drawdream-mcp-disc-"));
	try {
		resetMcpHubForTests();
		writeFileSync(
			join(dir, ".mcp.json"),
		...
		assert.ok(alpha);
		assert.equal(alpha!.command, "node"); // drawdream 覆盖 .mcp.json
		assert.ok(alpha!.sources.includes("project-mcp") || alpha!.sources.includes("drawdream"));
...
test("discover from real ~/.claude.json if present", () => {
	...
	const cat = discoverMcpCatalog(mkdtempSync(join(tmpdir(), "drawdream-mcp-home-")));
