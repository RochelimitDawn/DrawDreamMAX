...
test("解析：首行元数据、角色分配、is_system 跳过、swipe 回退", () => {
	...
	assert.equal(messages[2].source?.swipeId, 1);
});

test("解析：保留酒馆消息 metadata、MVU variables 与 swipes sidecar", () => {
	const { messages } = parseStChat([
		header,
		line({
			name: "青梧",
			is_user: false,
			mes: "正文",
			send_date: "2025-01-01T00:00:00.000Z",
			extra: { mood: "calm" },
	...
	assert.deepEqual(messages[0].source, {
		index: 1,
		sendDate: "2025-01-01T00:00:00.000Z",
		extra: { mood: "calm" },
		variables: { stat_data: { hp: 10 } },
		metadata: { source: "tavern" },
		swipes: ["正文", "另一版"],
