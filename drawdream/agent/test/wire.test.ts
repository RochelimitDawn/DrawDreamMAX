...
test("助手多文本块原样拼接并保留跨块 RP 标签", () => {
	const expected = "<char>黑川修</char><innervoice>保持沉默</innervoice>[clue title:钥匙]黄铜钥匙[/clue]";
	const history = toAssistantHistory([
		{
			role: "assistant",
			content: [
				{ type: "text", text: "<ch" },
				{ type: "text", text: "ar>黑川修</char><innervoice>保持沉默</innervoice>" },
				{ type: "text", text: "[clue title:钥匙]黄铜钥匙[/clue]" },
			],
		},
	]);
	assert.equal(history[0]?.text, expected);
});
...
test("toWireHistory 为消息保留稳定会话 ID 和索引", () => {
	const wired = toWireHistory([
		{ id: "entry-user", role: "user", content: "你好" },
		{ role: "assistant", content: "你好，旅人。" },
	], names);
	assert.equal(wired[0]?.id, "entry-user");
	assert.equal(wired[0]?.index, 0);
	assert.equal(wired[1]?.id, "session-message-1");
	assert.equal(wired[1]?.index, 1);
});
...
test("toAssistantHistory：工具轮还原为最终 assistant 的 activities", () => {
	const history = [
		{ role: "user", content: "查一下现在时间再搜" },
		{
			role: "assistant",
			content: [
				{ type: "thinking", thinking: "先取时" },
				{ type: "toolCall", id: "t1", name: "world_time", arguments: { timezone: "Asia/Shanghai" } },
			],
		},
		{
			role: "toolResult",
			toolName: "world_time",
			content: [{ type: "text", text: "2026-07-20 12:00 Asia/Shanghai" }],
		},
		{
			role: "assistant",
			content: [
				{ type: "toolCall", id: "t2", name: "smart_search", arguments: { query: "2026 news" } },
			],
		},
		{
			role: "toolResult",
			toolName: "smart_search",
			content: [{ type: "text", text: "1. 标题\n摘要…" }],
		},
		{
			role: "assistant",
			content: [{ type: "text", text: "现在是 2026 年 7 月 20 日，检索结果如下。" }],
		},
	];
	const list = toAssistantHistory(history);
	assert.equal(list.length, 2);
	assert.equal(list[0]?.role, "user");
	assert.equal(list[1]?.role, "assistant");
	assert.equal(list[1]?.text, "现在是 2026 年 7 月 20 日，检索结果如下。");
	const acts = list[1]?.activities ?? [];
	assert.equal(acts.length, 4);
	assert.deepEqual(
		acts.map((a) => `${a.kind}:${a.name}`),
		["tool_start:world_time", "tool_end:world_time", "tool_start:smart_search", "tool_end:smart_search"],
	);
	assert.ok(acts[0]?.detail?.includes("Asia/Shanghai"));
	assert.ok(acts[1]?.detail?.includes("2026-07-20"));
	assert.equal(acts[2]?.query, "2026 news");
	assert.equal(acts[1]?.isError, false);
});

test("toWireHistory：工具活动挂到最终 narrative 气泡", () => {
	const history = [
		...
		{
			role: "toolResult",
			toolName: "world_state_get",
			content: [{ type: "text", text: "（尚无记录）" }],
		},
		{ role: "assistant", content: [{ type: "text", text: "*她推门进来。*" }] },
	];
	const wired = toWireHistory(history, names);
	assert.deepEqual(
		wired.map((m) => m.channel),
	...
	const acts = wired[1]?.activities ?? [];
	assert.equal(acts.length, 2);
	assert.equal(acts[0]?.kind, "tool_start");
	assert.equal(acts[0]?.name, "world_state_get");
	assert.equal(acts[1]?.kind, "tool_end");
	assert.ok(acts[1]?.detail?.includes("尚无记录"));
});
