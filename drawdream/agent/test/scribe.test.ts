...
test("场记提示词：includeSummary 时要求 summary_entry", () => {
	const { systemPrompt } = buildScribeTurnPrompt(
		...
		{ includeSummary: true, turnNumber: 3 },
	);
	assert.ok(systemPrompt.includes("summary_entry"));
	assert.ok(systemPrompt.includes("第3轮"));
});
...
test("场记输出解析：只取 patch，丢弃审查字段", () => {
	...
	assert.equal(bare.summaryEntry, null);
...
test("场记输出解析：summary_entry", () => {
	const hit = parseScribeResult(
		'{"patch":{},"summary_entry":"[第2轮] 用户意图：要房 | 叙事要点：青梧安排客房"}',
	);
	assert.ok(hit);
	assert.equal(hit.summaryEntry, "[第2轮] 用户意图：要房 | 叙事要点：青梧安排客房");

	const arr = parseScribeResult('{"patch":{},"summary_entries":["[第1轮] 用户意图：问好 | 叙事要点：寒暄"]}');
	assert.ok(arr);
	assert.ok(arr.summaryEntry?.includes("问好"));
});
