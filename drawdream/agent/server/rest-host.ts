		...
		async setThinkingLevel(level) {
			...
			if (cur && cur.provider !== "unknown" && cur.id !== "unknown") {
				const m = session.modelRegistry.find(cur.provider, cur.id) as
					...
					| null;
				if (m && !m.reasoning) {
					const cacheKey = probeCacheKey(cur.provider, cur.id);
					const cached = thinkingProbeCache.get(cacheKey);
					if (cached && cached.length) {
						applyProbeCapability(cur.provider, cur.id, cached);
					} else if (lv !== "off" && !probingSet.has(cacheKey)) {
						// 无缓存且目标非 off：同步真实探测（写回能力 + 写缓存），
						// 否则切换档位仍被 clamp 回退。避免与 selectModel 探测并发。
						probingSet.add(cacheKey);
						try {
							await runThinkingProbe(cur.provider, cur.id);
						} finally {
							probingSet.delete(cacheKey);
						}
					}
