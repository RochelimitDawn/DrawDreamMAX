/**
 * DrawDream Agent 配置体系：
 * - 仓库 drawdream-profiles/*.json
 * - 当前启用 drawdream.agent.json
...
export function projectConfigDir(cwd: string): string {
	return join(cwd, ".drawdream");
}


export const AGENT_CONFIG_FILE = "drawdream.agent.json";
export const PROFILES_DIR = "drawdream-profiles";
export const ACTIVE_META_FILE = "drawdream.agent.meta.json";
...
export interface DrawdreamAgentConfig {
	version: 1;
	/** 默认渠道名（providers 键） */
...
export const EMPTY_AGENT_CONFIG = (): DrawdreamAgentConfig => ({
	version: 1,
	providers: {},
});
...
export function profilesDirPath(cwd: string): string {
...
export function normalizeAgentConfig(raw: unknown): DrawdreamAgentConfig {
...
export function loadAgentConfig(cwd: string): { path: string; exists: boolean; config: DrawdreamAgentConfig } {
...
export function saveAgentConfig(cwd: string, config: DrawdreamAgentConfig): void {
	const path = agentConfigPath(cwd);
...
/**
 * 把 DrawDream Agent 配置投影到 runtime 读取的文件（实现细节）。
 * - providers → agentDir/models.json
 * - 默认模型/思考/shell/skills → 项目 .drawdream/settings.json（合并）
 * - 默认模型/思考 → agentDir/settings.json（合并，影响全局默认）
 */
/** 中转站常见风控会拦默认 Node/OpenAI SDK UA；补浏览器 UA 提升连通率 */
const RELAY_DEFAULT_HEADERS: Record<string, string> = {
	"User-Agent":
		"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
	Accept: "application/json",
};

function withRelayHeaders(providers: Record<string, AgentProvider>): Record<string, AgentProvider> {
	...
	for (const [name, p] of Object.entries(providers)) {
		const existing =
			p.headers && typeof p.headers === "object" && !Array.isArray(p.headers)
				...
				: {};
		// 用户显式配置的 headers 优先；缺省再补中转友好头
		out[name] = {
			...p,
			headers: { ...RELAY_DEFAULT_HEADERS, ...existing },
		};
	}
	return out;
}

export function syncAgentConfigToRuntime(cwd: string, agentDir: string, config: DrawdreamAgentConfig): void {
	const cfg = normalizeAgentConfig(config);
	const providers = withRelayHeaders(cfg.providers);

	// providers → models.json
	writeJsonBackup(join(agentDir, "models.json"), { providers });

	// 同步明文 Key 到 auth.json，避免仅 models.json 回退路径在部分 runtime 状态下读不到
	// （会话默认 unknown 模型时 getApiKey 会按 provider 查 auth）
	syncLiteralKeysToAuthJson(agentDir, cfg.providers);
	...
	if (Object.keys(projectSettings).length > 0) {
		patchSettings(join(projectConfigDir(cwd), "settings.json"), projectSettings);
	}
...
/** 把渠道明文 apiKey 写入 agentDir/auth.json（AuthStorage 优先读此文件） */
function syncLiteralKeysToAuthJson(agentDir: string, providers: Record<string, AgentProvider>): void {
	const authPath = join(agentDir, "auth.json");
	let cur: Record<string, unknown> = {};
	if (existsSync(authPath)) {
		try {
			const raw = JSON.parse(readFileSync(authPath, "utf8")) as unknown;
			if (raw && typeof raw === "object" && !Array.isArray(raw)) cur = raw as Record<string, unknown>;
		} catch {
			cur = {};
		}
	}
	...
	for (const [name, p] of Object.entries(providers)) {
		...
		if (km.keyKind !== "literal" || !km.hasKey) continue;
		const prev = cur[name];
		const prevKey =
			prev && typeof prev === "object" && !Array.isArray(prev) && (prev as { type?: string }).type === "api_key"
				? String((prev as { key?: string }).key ?? "")
				: "";
		if (prevKey === key) continue;
		cur[name] = { type: "api_key", key };
		changed = true;
	}
	if (changed || !existsSync(authPath)) {
		writeJsonBackup(authPath, cur);
	}
}
 ...
 * 返回是否发生了改写。
 */
export function materializeEnvKeysInConfig(config: DrawdreamAgentConfig): boolean {
	let changed = false;
...
export function saveProfile(cwd: string, id: string, name: string, config: DrawdreamAgentConfig): AgentProfileRecord {
...
export function enableProfile(cwd: string, agentDir: string, id: string): DrawdreamAgentConfig {
	const rec = loadProfile(cwd, id);
...
/**
 * 把当前 drawdream.agent.json 拆进仓库（若仓库为空）。
...
export function migrateActiveConfigIntoProfiles(cwd: string): { migrated: boolean; ids: string[] } {
	...
	for (const [pname, provider] of Object.entries(config.providers)) {
		...
		const cfg: DrawdreamAgentConfig = {
			version: 1,
			...extras,
