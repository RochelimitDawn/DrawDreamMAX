...
/** 是否为 ST 聊天补全预设（含 prompts / prompt_order） */
export function isStPresetJson(raw: unknown): raw is Record<string, unknown> {
	if (!raw || typeof raw !== "object") return false;
	const o = raw as Record<string, unknown>;
	return Array.isArray(o.prompts) || Array.isArray(o.prompt_order);
}

export function summarizeConvertReport(
	report: ReportItem[],
	preset: RpPreset,
): PresetConvertSummary {
	const count = (pred: (a: ReportItem["action"]) => boolean) =>
		report.filter((r) => pred(r.action)).length;
	return {
		system: count((a) => a === "system"),
		postHistory: count((a) => a === "postHistory"),
		marker: count((a) => a.startsWith("marker")),
		disabled: count((a) => a.startsWith("禁用")),
		missing: count((a) => a === "缺失定义"),
