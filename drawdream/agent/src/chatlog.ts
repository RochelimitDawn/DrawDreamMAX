...
export function parseStChat(jsonlText: string): ParsedStChat {
	...
	for (let i = 1; i < lines.length; i++) {
		...
		const source: StChatMessage["source"] = {
			index: i,
			...(typeof m.send_date === "string" ? { sendDate: m.send_date } : {}),
			...(Object.prototype.hasOwnProperty.call(m, "extra") ? { extra: m.extra } : {}),
			...(Object.prototype.hasOwnProperty.call(m, "variables") ? { variables: m.variables } : {}),
			...(Object.prototype.hasOwnProperty.call(m, "metadata") ? { metadata: m.metadata } : {}),
			...(Array.isArray(m.swipes) ? { swipes: m.swipes.filter((x): x is string => typeof x === "string") } : {}),
			...(typeof m.swipe_id === "number" ? { swipeId: m.swipe_id } : {}),
			originalText: text,
		};
		messages.push({
