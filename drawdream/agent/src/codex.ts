/**
 * DrawDream 知识库：可命名、可跨会话挂载的设定库（.drawdream-codex/）。
 * 与按卡的补充设定（.drawdream-lore/）分离；条目复用 LorebookEntry，可导出 ST 兼容世界书。
 */
...
export function createCodex(cwd: string, name: string, description = ""): CreateCodexResult {
	...
	writeCodexFile(path, { format: "drawdream-codex", version: 1, name: n, description: description.trim(), entries: [] });
...
export function appendCodexEntry(cwd: string, name: string, input: CodexEntryInput): AppendCodexResult {
	...
	const file = readCodexFile(meta.file) ?? { format: "drawdream-codex", version: 1, name: meta.name, entries: [] };
