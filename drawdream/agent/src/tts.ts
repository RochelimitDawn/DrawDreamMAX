/**
 * 文生音：OpenAI 兼容 /v1/audio/speech + 本地 .drawdream-audio/ 落盘（纯函数侧，零 pi 依赖）。
 *
 * 配置（任选其一即可）：
 * - DRAWDREAM_TTS_BASE_URL + DRAWDREAM_TTS_API_KEY
 * - 或 OPENAI_API_KEY（base 默认 https://api.openai.com/v1）
 * 可选：DRAWDREAM_TTS_MODEL（默认 tts-1）、DRAWDREAM_TTS_VOICE（默认 alloy）
 */
...
export function loadTtsConfig(env: NodeJS.ProcessEnv = process.env): TtsConfig | null {
	const apiKey = (env.DRAWDREAM_TTS_API_KEY || env.OPENAI_API_KEY || "").trim();
	if (!apiKey) return null;
	const rawBase = (env.DRAWDREAM_TTS_BASE_URL || env.OPENAI_BASE_URL || "https://api.openai.com/v1").trim().replace(/\/+$/, "");
	// 允许用户填到 /v1 或不带 /v1
	const baseUrl = rawBase.endsWith("/v1") ? rawBase : `${rawBase}/v1`;
	return {
		baseUrl,
		apiKey,
		model: (env.DRAWDREAM_TTS_MODEL || "tts-1").trim(),
		voice: (env.DRAWDREAM_TTS_VOICE || "alloy").trim(),
	};
}

export function ttsConfigHint(): string {
	return "未配置文生音。请设置环境变量 DRAWDREAM_TTS_API_KEY（或 OPENAI_API_KEY），可选 DRAWDREAM_TTS_BASE_URL / DRAWDREAM_TTS_MODEL / DRAWDREAM_TTS_VOICE。也可用外部技能生成音频文件后调用 show_audio 交付。";
}
...
export function audioDir(cwd: string): string {
	return join(cwd, ".drawdream-audio");
}

/** 内容寻址写入 .drawdream-audio/，返回可供前端使用的 /audio/ 路径 */
...
/** 复制已有本地文件进 .drawdream-audio（show_audio 用） */
