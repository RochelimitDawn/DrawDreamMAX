 ...
 *
 * PNG 提取按公开 Character Card V2/V3 规范独立实现：
...
import { normalizeCardRegexScripts } from "./card-regex.ts";
import { normalizeEntries } from "./lorebook.ts";
import { buildTavernRuntimeManifest } from "./card-runtime.ts";
import { substituteTavernMacros } from "./tavern-prompt.ts";
...
export function normalizeCard(input: unknown): CharacterCard {
	...
	const extensions = pick("extensions");
	const ext = extensions && typeof extensions === "object" ? extensions as Record<string, unknown> : {};
	const regexScripts = normalizeCardRegexScripts(ext.regex_scripts ?? ext.regexScripts);
	...
	const card: CharacterCard = {
		...
		book,
		compat: { regexScripts, unknownExtensions: { ...ext } },
	};
	card.runtimeManifest = buildTavernRuntimeManifest(card);
	return card;
}
...
export function applyMacros(text: string, ctx: MacroContext): string {
	return substituteTavernMacros(text, ctx).text;
}
