/**
 * DrawDream MCP 客户端与本机发现。
 * 源：~/.claude.json · ~/.drawdream/mcp.json · 项目 .mcp.json · .drawdream-mcp.json 等。
 * 默认关闭；按会话启用；工具名 mcp__{id}__{tool}。领域层不 import runtime。
 */
...
export const MCP_CONFIG_FILE = ".drawdream-mcp.json";
export const MCP_USER_CONFIG_FILE = "mcp.json"; // ~/.drawdream/mcp.json
export const RP_MCP_TYPE = "rp-mcp"; // 会话树：本对话启用的服务器 id 列表

export type McpTransport = "stdio" | "http" | "sse";

/** 配置从哪来的（面板展示；高优先级覆盖低优先级的 endpoint） */
export type McpSource = "claude" | "cursor" | "user" | "project-mcp" | "drawdream";
...
export function userMcpConfigPath(): string {
	return join(homedir(), ".drawdream", MCP_USER_CONFIG_FILE);
}

export function emptyMcpConfig(): McpConfigFile {
	return { format: "drawdream-mcp", version: 2, servers: [], defaults: {} };
}
...
export function saveUserMcpConfig(config: McpConfigFile): void {
	const dir = join(homedir(), ".drawdream");
...
export function normalizeMcpConfig(raw: Partial<McpConfigFile> | null | undefined): McpConfigFile {
	...
	return { format: "drawdream-mcp", version: 2, servers: out, defaults };
}
 ...
 * claude → cursor → user(~/.drawdream) → project .mcp.json → project .drawdream-mcp.json
 */
	...
	if (project.servers.length) {
		layers.push({
			...
			entries: project.servers.map((s) => ({
				...s,
				source: "drawdream" as const,
				sources: ["drawdream" as const],
				discovered: false,
			})),
		});
	}
...
async function openConnection(server: McpServerConfig, timeoutMs: number): Promise<{ client: Client }> {
	const client = new Client({ name: "drawdream", version: "0.1.0" });
