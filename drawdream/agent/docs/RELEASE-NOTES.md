> **注意（2.0）**：配置与数据主名已切换为 `drawdream.*` / `.drawdream-*`；下文中的 `liyuan.*` 为历史版本说明。

# DrawDream Alpha 2.0.0-alpha.1 · mobile.39
...
| 聊天 jsonl | ✅（聊天页导入，清洗后导入续玩并保留 metadata/MVU/swipe sidecar） |
| 预设 | ⚠️ 转换器可用，旧架构补偿块可能无效 |
| 卡内 `extensions.regex_scripts` | ✅（显示期安全替换） |
| MVU 变量命令 / STscript / 前端插件 | ⚠️ 原始数据保留，运行时命令后续实现 |
| 卡内 HTML/JS 前端 | ⚠️ 使用 DrawDream 安全 iframe 能力，酒馆插件 API 后续实现 |
...
- 不执行酒馆 JavaScript、STscript、TavernHelper 或前端插件 API
- 看图需要视觉模型  
