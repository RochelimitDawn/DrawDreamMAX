...
# 编辑 agent/drawdream.agent.json，填入 apiKey / 模型 id
...
**请勿把带 Key 的 `drawdream.agent.json` 提交或发群文件。**
...
- 程序卡 / `show_html`：沙箱 iframe（`allow-scripts`、无 same-origin），经 `DrawDream.send` / postMessage 桥回传用户消息
