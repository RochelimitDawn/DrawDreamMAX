# Pack DrawDream Agent release kits (Windows + Linux + macOS)
# - No node_modules (small zip; first-run npm install)
# - Product UI lives in parent drawdream/dist (set DRAWDREAM_UI_DIST when running)
...
Write-Host "[pack] DrawDream Agent $ver  root=$prod  target=$Target"
...
2. Edit ``drawdream.agent.json`` -> set ``apiKey`` and model id
3. Built-in demo card: ``assets/cards/default_Qingwu.json`` (DrawDream original; not from SillyTavern)
...
"@
    ...
    "linux" {
      $body = @"
# DrawDream Agent v$Version - Linux

## Quick start
``````bash
unzip DrawDream-$Version-linux.zip
cd DrawDream
chmod +x start.sh
./start.sh
``````
...
"@
    ...
    "macos" {
      $body = @"
# DrawDream Agent v$Version - macOS
...
``````bash
xattr -dr com.apple.quarantine /path/to/DrawDream
``````
...
"@
    ...
    default {
      $body = "# DrawDream Agent v$Version`n"
    }
  }
...
function Stage-Clean {
  ...
  $dest = Join-Path $StageRoot "DrawDream"
  ...
  $xd = @(
    ...
    ".git",
    ".github",
    ".drawdream", ".drawdream-artifacts", ".drawdream-assistant", ".drawdream-cache", ".drawdream-codex",
    ".drawdream-lore", ".drawdream-media", ".drawdream-skills", ".drawdream-state",
    ".drawdream-uploads", ".drawdream-audio", ".drawdream-worldline",
    ".drawdream-media", ".drawdream-uploads",
    ".playwright-mcp",
    "ab-test", "import-test", "drawdream-profiles", "assets/gen"
  )
  $xf = @(
    "*.log", "*.tsbuildinfo", ".DS_Store", "Thumbs.db",
    "*.bak",
    ".drawdream-personas.json",
    "drawdream.config.json", "drawdream.agent.json", "drawdream.agent.meta.json",
  ...
  # Keep only DrawDream default_* sample cards (never ship ST/community demo packs
  ...
  Copy-Item (Join-Path $dest "drawdream.config.example.json") (Join-Path $dest "drawdream.config.json") -Force
  Copy-Item (Join-Path $dest "drawdream.agent.example.json") (Join-Path $dest "drawdream.agent.json") -Force

  # Placeholder RELEASE - replaced per platform before zip
  Set-Content -LiteralPath (Join-Path $dest "RELEASE.txt") -Value "# DrawDream Agent v$ver" -Encoding UTF8
  return $dest
}
...
skip_dirs = {
  'node_modules', '.git', '.github', '.drawdream', '.drawdream-cache',
  '.drawdream-artifacts', '.drawdream-assistant', '.drawdream-codex', '.drawdream-lore',
...
"@
  $pyFile = Join-Path $env:TEMP ("drawdream-rel-zip-{0}.py" -f [guid]::NewGuid().ToString("n"))
...
function Build-PlatformZip {
  ...
  $platDest = Join-Path $platStage "DrawDream"
  ...
  $zip = Join-Path $OutDir "DrawDream-$Version-$Platform.zip"
...
$stage = Join-Path $env:TEMP "drawdream-release-$stamp"
