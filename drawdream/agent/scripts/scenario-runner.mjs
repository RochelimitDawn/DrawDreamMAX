...
const settingsPath = join(appDir, ".drawdream", "settings.json");
...
// 使用本地 fork 的 drawdream-agent（file:../drawdream-runtime），扩展在 .drawdream/extensions/
const child = spawn(
	"npx",
	["drawdream-agent", "--mode", "rpc", "-e", ".drawdream/extensions/roleplay.ts", "--model", model],
	{
		cwd: appDir,
...
child.on("exit", (code) => {
	if (!finished) {
		console.error(`drawdream-agent 进程提前退出（code ${code}）`);
		process.exit(1);
	}
});
