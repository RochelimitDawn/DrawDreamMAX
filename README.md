...
移动端优化的 Pi AI 角色扮演：内嵌 Node 运行时 + 绘梦 UI，记忆分层、抉择门禁、RP 结构化组件、世界线与 Agent 协作。
...
| **当前发布版本** | `v2.0.0-alpha.1-mobile.N ` |
...
| `/library` | 资料| 人设 / 世界书 / 预设入口 |
| `/novel-forge` | 工坊 | `/api/forge/*` |
...
| 产品 | **Alpha 2.0** |
| 包 | `2.0.0-alpha.1` |
| Release | **`v2.0.0-alpha.1-mobile.N`** |
| Agent | DrawDream Agent（Runtime 内嵌 Node） |
...
- **产品源码**曾引用参考 [梨园 Liyuan](https://github.com/weidu12123/Liyuan) 的角色扮演 / Agent 工作台设计源码；目前实现UX、UI、Runtime 与产品逻辑均为 DrawDream 独立维护。
...
本项目（含 UI 框架、DrawDream Agent 拓展、Runtime 交互层）采用：
...
**DrawDreamMAX** · Alpha 2.0 · Pi 驱动的酒馆工作台 · 方寸之间，绘梦天地
